...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    return res.status(500).json({ error: 'server_error', message: 'サーバーエラーが発生しました。' });
  }
});
...
app.get('/api/me', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query('SELECT username, wins, losses FROM users WHERE id = $1', [req.user.uid]);
    const user = result.rows[0];
...
async function recordResult(room) {
  ...
  try {
    if (winner) await pool.query('UPDATE users SET wins = wins + 1 WHERE id = $1', [winner.uid]);
    if (loser) await pool.query('UPDATE users SET losses = losses + 1 WHERE id = $1', [loser.uid]);
  } catch (e) {
...
initDb()
  ...
  .catch((e) => {
    console.error('Failed to initialize database. Check DATABASE_URL.', e);
    process.exit(1);
  });
